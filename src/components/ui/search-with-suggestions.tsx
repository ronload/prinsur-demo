...
export function SearchWithSuggestions({
  ...
  const generateSuggestions = (): SearchSuggestion[] => {
    ...
    if (searchTerm) {
      ...
      // 2. 匹配保險分類
      const categories =
        insuranceCategories[locale as keyof typeof insuranceCategories] ||
        insuranceCategories["zh-TW"];
      ...
      if (remainingSlots > 0) {
        ...
        if (companyCount > 0) {
          const shuffledCompanies = [...insuranceCompanies].sort(
            () => Math.random() - 0.5,
          );
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      {/* dropdown只在有建议时显示 */}
      {isOpen && suggestions.length > 0 && (
        <div data-dropdown-content className={dropdownClasses}>
