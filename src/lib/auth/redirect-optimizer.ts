...
export function getOptimalRedirect(
  ...
  const applicableRules = REDIRECT_RULES.filter(
    (rule) => rule.fromRole === user.type && rule.condition(user, context),
...
const REDIRECT_RULES: RedirectRule[] = [
  // High priority: First-time users should complete profile
  {
    ...
    condition: (user, context) => {
      ...
      return (
        context.currentPath.includes("/app") &&
        !context.currentPath.includes("/profile")
      );
    },
  ...
  // Low priority: Time-based redirects (business hours optimization)
  {
    ...
    condition: (user, context) => {
      ...
      return (
        hour >= 9 && hour <= 17 && context.currentPath.includes("/dashboard")
      );
    },
    redirect: (locale, user, context) => `/${locale}/workspace/clients`,
    priority: 30,
...
export function logRedirect(
  ...
  console.log(
    `[REDIRECT] User ${user.email} (${user.type}) redirected from ${fromPath} to ${toPath}. Reason: ${reason}`,
  );
}
